...
class Prestamo:
    def __init__(self,empleado, aamm, valor, numPagos,saldo, estado= True,id=1):
        ...
        self.cuota = float(valor)/int(numPagos)
        self.saldo = saldo
        self.estado = estado
    ...
    def getPrestamo(self):
       return [str(self.id),str(self.empleado.id),self.aamm,str(self.valor),str(self.numPagos),str(self.cuota),str(self.saldo),str(self.estado)]    
...
class DetalleNomima(CalculoRol):
    ...
    def getSobretiempo(self,aamm):
        ...
        if self.empleado.id[0]=="O":
            ...
            sobre = archiSobretiempo.buscar2(self.empleado.id,aamm)
            if sobre:
                entSobretiempo = Sobretiempo(sobre[1],sobre[2],float(sobre[3]),float(sobre[4]),sobre[5],sobre[0])
                return round(self.empleado.valorHora()+(entSobretiempo.h50*1.5+entSobretiempo.h100*2),2)
        return calSobre
    
    def getAntiguedad(self,deduccion):
        calAnti = 0
        if self.empleado.id[0]=="O":
            calAnti =round(deduccion.getAntiguedad()*(date.today().year-self.empleado.fechaIngreso)/365*self.empleado.sueldo,2)
        return calAnti
